  ...
  return (
    <div className="w-full space-y-6 pb-10">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="overflow-x-auto"><table className="w-full min-w-[760px] border-collapse text-left text-sm"><thead><tr className="border-b border-slate-200 bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500"><th className="px-5 py-4 text-center">No.</th><th className="px-5 py-4">Nama Kategori</th><th className="px-5 py-4">Trigger Hari & Jam</th><th className="px-5 py-4 text-right">Biaya Tambahan</th><th className="px-5 py-4 whitespace-nowrap">Created At</th><th className="px-5 py-4">Created By</th><th className="px-5 py-4 whitespace-nowrap">Updated At</th><th className="px-5 py-4">Updated By</th><th className="px-5 py-4 whitespace-nowrap">Deleted At</th><th className="px-5 py-4">Deleted By</th><th className="px-5 py-4 text-center">Aksi</th></tr></thead><tbody className="divide-y divide-slate-100">{loading ? <tr><td colSpan="11" className="px-5 py-10 text-center text-slate-500">Memuat data kategori tarif...</td></tr> : pageItems.length === 0 ? <tr><td colSpan="11" className="px-5 py-10 text-center text-slate-500">Kategori tarif tidak ditemukan.</td></tr> : pageItems.map((item, index) => <tr key={item.id_kategori_tarif} className="hover:bg-slate-50"><td className="px-5 py-4 text-center text-slate-500">{(page - 1) * ITEMS_PER_PAGE + index + 1}</td><td className="px-5 py-4"><p className="font-semibold text-slate-900">{item.nama_kategori}</p>{item.is_default && <span className="mt-1 inline-flex rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-semibold text-emerald-700">Default</span>}</td><td className="px-5 py-4 text-slate-600">{item.is_default ? 'Fallback jika tidak ada trigger' : item.hari_berlaku?.length && item.jam_mulai && item.jam_selesai ? `${item.hari_berlaku.map((day) => day.slice(0, 3)).join(', ')} | ${item.jam_mulai.slice(0, 5)}-${item.jam_selesai.slice(0, 5)}` : 'Belum ada trigger'}</td><td className="px-5 py-4 text-right font-semibold text-slate-800">{formatRupiah(item.biaya_tambahan)}</td><td className="px-5 py-4 text-xs text-slate-500 whitespace-nowrap">{formatDate(item.created_at ?? item.createdAt)}</td><td className="px-5 py-4 text-xs text-slate-500">{formatUser(item.created_by ?? item.created_by_user ?? item.createdBy)}</td><td className="px-5 py-4 text-xs text-slate-500 whitespace-nowrap">{formatDate(item.updated_at)}</td><td className="px-5 py-4 text-xs text-slate-500">{formatUser(item.updated_by ?? item.updated_by_user)}</td><td className="px-5 py-4 text-xs text-slate-500 whitespace-nowrap">{formatDate(item.deleted_at)}</td><td className="px-5 py-4 text-xs text-slate-500">{formatUser(item.deleted_by ?? item.deleted_by_user)}</td><td className="px-5 py-4"><div className="flex justify-center gap-2"><button type="button" onClick={() => openEdit(item)} title="Edit kategori" className="rounded-lg border border-slate-200 bg-white p-2 text-slate-600 hover:bg-slate-50"><FaEdit /></button><button type="button" onClick={() => handleDelete(item)} title="Hapus kategori" className="rounded-lg border border-red-200 bg-red-50 p-2 text-red-600 hover:bg-red-100"><FaTrash /></button></div></td></tr>)}</tbody></table></div>
