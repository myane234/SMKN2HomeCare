...
import { FaDownload, FaSyncAlt, FaExclamationTriangle, FaUserMd, FaFilter } from 'react-icons/fa';
import Pagination from '../components/pagination';
// 1. Impor exportLaporanNakes
import { getLaporanNakes, exportLaporanNakes } from '../data/laporanData';

export default function PageLaporanNakes() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Gagal memuat data kinerja nakes.');
      setLaporanData([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = async (format = 'csv') => {
    try {
      ...
      const blob = await exportLaporanNakes(params, format);

      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `laporan_nakes_${new Date().toISOString().slice(0, 10)}.${format === 'xlsx' ? 'xlsx' : 'csv'}`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(url);
    } catch (err) {
      alert('Gagal mengunduh laporan nakes: ' + err.message);
    } finally {
      setExporting(false);
    }
  };
    ...
    <div className="w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 flex items-center gap-2.5">
            <FaUserMd className="text-primary" /> Laporan Kinerja Tenaga Medis
          </h1>
          <p className="text-xs sm:text-sm text-slate-500 mt-1">Performa, tugas, dan akumulasi hak tenaga medis secara menyeluruh</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-1">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Tenaga Medis</span>
          <p className="text-3xl font-extrabold text-slate-900">{summaryData?.total_nakes || 0}</p>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-1">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Order Selesai</span>
          <p className="text-3xl font-extrabold text-emerald-600">{summaryData?.total_order_selesai || 0}</p>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-1">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Akumulasi Hak Nakes</span>
          <p className="text-3xl font-extrabold text-blue-600">{formatRupiah(summaryData?.total_akumulasi_hak_nakes)}</p>
        </div>
      </div>

      {/* Filter Card */}
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-xs">
        <form onSubmit={handleApplyFilter} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 items-end">
          <div>
          ...
          <div>
            ...
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Nama / No STR..." className="w-full px-3 py-2 text-xs border border-slate-200 rounded-xl" />
          </div>
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="bg-slate-50/70 border-b border-slate-200 text-slate-500 font-bold uppercase tracking-wider text-[11px]">
                <th className="p-3.5">Nama Nakes</th>
                <th className="p-3.5">Profesi</th>
                <th className="p-3.5">Nomor STR</th>
                <th className="p-3.5 text-center">Total Order</th>
                <th className="p-3.5 text-center">Selesai</th>
                <th className="p-3.5 text-center">Batal</th>
                <th className="p-3.5 text-right">Akumulasi Hak</th>
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {loading ? (
                ...
                <tr><td colSpan="8" className="text-center py-8 text-slate-400">Tidak ada data kinerja nakes.</td></tr>
              ) : (
                laporanData.map((item, idx) => (
                  <tr key={item.id_tenaga_medis || idx} className="hover:bg-slate-50/80 transition-colors">
                    <td className="p-3.5 font-bold text-slate-900">{item.nama_lengkap || '-'}</td>
                    <td className="p-3.5 text-slate-600">{item.profesi || '-'}</td>
                    <td className="p-3.5 font-mono font-semibold text-slate-700">{item.nomor_str || '-'}</td>
                    <td className="p-3.5 text-center font-bold text-slate-800">{item.total_order || 0}</td>
                    <td className="p-3.5 text-center font-bold text-emerald-600">{item.total_selesai || 0}</td>
                    <td className="p-3.5 text-center font-bold text-red-500">{item.total_batal || 0}</td>
                    <td className="p-3.5 text-right font-bold text-blue-600">{formatRupiah(item.total_hak_nakes)}</td>
                    <td className="p-3.5 text-center font-bold text-amber-500">★ {item.rating || 0}</td>
                  </tr>
                ))
              )}
            </tbody>
