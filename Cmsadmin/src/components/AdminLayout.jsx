...
<div className="relative" ref={menuRef}>
  ...
  >
    {avatarSrc ? (
      ...
      <div className="flex h-9 w-9 items-center justify-center rounded-full bg-emerald-100 font-bold text-emerald-800 text-sm">
  ...
  {open && (
    <div className="absolute right-0 top-[calc(100%+8px)] w-48 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl shadow-slate-200/60 z-50 space-y-1">
      {/* Tombol ke Admin Profile (Endpoint: /profile) */}
      <Link
        to="/profile"
        onClick={() => setOpen(false)}
        className="flex w-full items-center gap-2.5 rounded-xl px-3.5 py-2.5 text-left text-[13px] font-semibold text-slate-700 hover:bg-slate-100 transition-colors"
      >
        <svg
