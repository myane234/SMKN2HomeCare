...
import { requestOtp, verifyOtp, resetPassword } from "@/services/forgotPasswordService";
...
export default function ForgotPasswordPage() {
  ...
  // Steps: 1 = Input Email, 2 = Verify OTP, 3 = Reset Password, 4 = Selesai
  ...
  const handleRequestOtp = async (e) => {
    ...
    try {
      ...
      setStep(2);
      setResendTimer(60); // 60 detik cooldown
    } catch (err) {
      setErrorMsg(err.message || "Gagal meminta kode OTP. Silakan coba lagi.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await verifyOtp(email.trim().toLowerCase(), trimmedOtp);
      if (res.reset_token) {
        setResetToken(res.reset_token);
        setSuccessMsg(res.message || "Kode OTP valid.");
        setStep(3);
      } else {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      setSuccessMsg(res.message || "Password berhasil direset!");
      setStep(4);
    } catch (err) {
  ...
  const renderContent = () => (
    <div className="w-full bg-white rounded-3xl p-6 sm:p-8 shadow-2xl border border-slate-100">
      ...
      {step === 1 && (
        <div>
          ...
          <p className="mb-5 text-xs sm:text-sm text-slate-500 font-medium">
            Masukkan email terdaftar akun Anda. Kami akan mengirimkan 6-digit kode OTP untuk verifikasi.
          </p>
          ...
          <form onSubmit={handleRequestOtp} className="space-y-4">
            ...
            >
              {loading ? "Mengirim Kode OTP..." : "Kirim Kode OTP"}
            </button>
          </form>
        </div>
      )}
      ...
      {step === 2 && (
        <div>
          <h2 className="text-2xl font-black text-slate-800 mb-1">Verifikasi OTP</h2>
      ...
      {step === 3 && (
        <div>
          <h2 className="text-2xl font-black text-slate-800 mb-1">Password Baru</h2>
          <p className="mb-5 text-xs sm:text-sm text-slate-500 font-medium">
      ...
      <div className="mt-6 border-t border-slate-100 pt-4 text-center">
        <p className="text-xs text-slate-600">
          ...
          <Link href="/login" className="font-bold text-[#004fa4] hover:underline">
            Masuk
          </Link>
        </p>
      </div>
      ...
      <div className="flex lg:hidden w-full min-h-screen flex-col relative bg-white">
        <div className="w-full bg-gradient-to-br from-[#0284c7] via-[#004fa4] to-[#2dd4bf] px-6 pt-10 pb-20 flex flex-col items-start justify-start rounded-b-[40px] shadow-lg text-left">
          ...
          <h1 className="text-xl sm:text-2xl font-black tracking-tight leading-snug text-white drop-shadow-md mb-2">
            Pemulihan <span className="text-yellow-200">Kata Sandi</span>
          </h1>
          <p className="text-white/90 text-xs leading-relaxed font-medium max-w-sm">
            Kembalikan akses ke akun layanan kesehatan SmartHomeCare Anda dengan mudah dan aman.
          </p>
        </div>
        ...
        <div className="w-1/2 bg-gradient-to-br from-[#0284c7] via-[#004fa4] to-[#2dd4bf] text-white px-16 py-16 xl:px-20 xl:py-20 flex flex-col justify-start rounded-r-[140px] shadow-lg relative">
          <div className="max-w-xl">
            ...
            <h1 className="text-4xl xl:text-5xl font-black tracking-tight leading-tight mb-5 text-white drop-shadow-md">
              Pemulihan <br />
              <span className="text-yellow-200 drop-shadow-lg">Akses Akun Anda</span>
            </h1>
            <p className="text-white/95 text-base xl:text-lg leading-relaxed font-medium drop-shadow-sm">
              Kami menjaga keamanan data dan akun medis Anda dengan sistem verifikasi kode OTP satu kali pakai yang langsung dikirimkan ke email terdaftar Anda.
            </p>
          </div>
        </div>
