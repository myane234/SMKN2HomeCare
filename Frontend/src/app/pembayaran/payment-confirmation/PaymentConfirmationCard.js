...
export default function PaymentConfirmationCard({ status = "success", data = {} }) {
  ...
  const isBiayaTambahan = Boolean(data?.isBiayaTambahan) || searchParams.get("type") === "bhp";
  ...
  const urlOrderId =
    initialUrlOrderId ||
    (searchParams.get("booking_id") ? `BOOKING-${searchParams.get("booking_id")}` : "-");

  const urlMetode = searchParams.get("metode") || "QRIS / Transfer";

  const resolvedPrice = data?.price && Number(data.price) > 0 ? Number(data.price) : urlTotal;

  let serviceNameDisplay = data?.serviceName || "Layanan Kesehatan Home Care";

  if (!isBiayaTambahan && Array.isArray(data?.layanan) && data.layanan.length > 0) {
    serviceNameDisplay = data.layanan
      .map((item) => item.nama_layanan || item.nama)
      .filter(Boolean)
      .join(", ");
  }

  if (isBiayaTambahan) {
    serviceNameDisplay = "Biaya Tambahan";
  }
  ...
  const orderData = {
    ...
    charge: isBiayaTambahan ? 0 : Number(data?.charge || 0),

    fees: isBiayaTambahan ? 0 : Number(data?.fees || 0),

    status: data?.status || (isSuccess ? "Lunas" : "Menunggu Konfirmasi"),
  };

  const totalPrice = isBiayaTambahan
    ? orderData.price
    : orderData.price + orderData.charge + orderData.fees;
    ...
    <div className="relative min-h-[calc(100vh-5rem)] w-full bg-[#6c9bf3] flex items-center justify-center p-4 sm:p-6 overflow-x-hidden">
      <div className="w-full max-w-md bg-white rounded-[32px] shadow-2xl p-6 sm:p-8 relative flex flex-col my-6">
        <div className="flex items-center justify-between pb-4">
          <Link href="/" className="text-slate-400 hover:text-slate-600 transition-all" title="Keluar">
            <X className="w-5 h-5" />
          </Link>
        ...
        <div className="mt-6 space-y-3 text-xs">
          <h2 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">DETAIL TRANSAKSI</h2>
          ...
          <div className="space-y-3 text-xs">
            <div className="flex items-center justify-between py-0.5">
              ...
              <div className="flex items-center gap-1 font-bold text-slate-900">
                ...
                {orderData.orderId !== "-" && (
                  <button type="button" onClick={handleCopyOrder} className="text-blue-600 hover:text-blue-700 transition ml-0.5" title="Salin No.Transaksi">
            ...
            <div className="flex items-center justify-between py-0.5">
              ...
              <span className="font-semibold text-slate-900 text-right">{orderData.paymentTime}</span>
            </div>

            <div className="flex items-center justify-between py-0.5">
              <span className="text-slate-500 font-medium">Metode</span>
              <span className="font-bold text-slate-900 uppercase">{orderData.paymentMethod}</span>
            </div>

            <div className="flex items-center justify-between py-2 border-b border-slate-100">
              <span className="text-slate-500 font-medium">Status</span>
              <span className={`px-2 py-0.5 text-[11px] font-bold rounded-md ${isSuccess ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                {orderData.status}
              </span>
            </div>
          </div>
        ...
        <div className="mt-6 space-y-3">
          <div className="flex justify-between items-start gap-4">
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">
              {isBiayaTambahan ? "BIAYA TAMBAHAN" : "RINCIAN LAYANAN"}
            </span>
          </div>

          <div className="flex justify-between items-start gap-4">
            <p className="text-xs font-bold text-slate-800 leading-relaxed max-w-[260px]">
        ...
        <div className="flex items-center justify-between py-0.5">
          <span className="text-xs font-extrabold text-slate-900">
            {isBiayaTambahan ? "Total Biaya Tambahan" : "Total Pembayaran"}
          </span>
        ...
        <div className="mt-6 space-y-2.5">
          ...
          <Link href="/" className="w-full text-slate-600 hover:text-slate-900 font-semibold py-2.5 flex items-center justify-center gap-1.5 text-xs transition">
        ...
        <div className="fixed inset-0 z-[999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl relative border border-slate-200 my-8">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              ...
              <div className="flex items-center gap-2">
                <button type="button" onClick={handlePrintReceipt} className="p-2 rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-100 transition flex items-center gap-1 text-xs font-semibold" title="Cetak Receipt">
                  <Printer className="w-4 h-4" />
                  <span className="hidden sm:inline">Cetak / PDF</span>
                </button>

                <button type="button" onClick={() => setShowReceiptModal(false)} className="p-2 rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200 transition">
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>
            ...
            <div className="mt-5 p-5 bg-slate-50 rounded-2xl border border-slate-200/80 font-sans">
              ...
              <div className="py-4 border-b border-dashed border-slate-300 space-y-1.5 text-xs text-slate-600">
                <h2 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-3">DETAIL TRANSAKSI</h2>
            ...
            <div className="mt-5 text-center">
              <button type="button" onClick={() => setShowReceiptModal(false)} className="w-full bg-slate-900 hover:bg-slate-800 text-white font-semibold py-3 rounded-2xl text-sm transition">
                Tutup Struk
              </button>
            </div>
          </div>
