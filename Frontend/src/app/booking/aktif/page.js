...
function normalizeBiayaTambahanResponse(response, fallback = null) {
  ...
  const value = data?.biaya_tambahan_pasien ?? data?.biaya_tambahan ?? data;

  if (
    value &&
    typeof value === "object" &&
    !Array.isArray(value) &&
    (value.nominal != null || value.nominal_format != null || value.status_transaksi != null)
  ) {
    return value;
  }
  return fallback;
        ...
        return (
          <div key={s.label} className="flex items-start gap-3">
            <div className="flex flex-col items-center">
              <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center shrink-0 transition-all duration-300 ${done ? "bg-emerald-500" : active ? "bg-emerald-600 ring-4 ring-emerald-100" : "bg-slate-100"}`}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${done || active ? "text-white" : "text-slate-400"}`}>
            ...
            <div className="pt-1 sm:pt-1.5 pb-4 sm:pb-5">
              <p className={`text-xs sm:text-sm font-semibold leading-tight ${active ? "text-slate-900" : done ? "text-emerald-600" : "text-slate-400"}`}>{s.label}</p>
              {active && <p className="text-[11px] sm:text-xs text-slate-500 mt-0.5">Sedang berlangsung</p>}
            </div>
          </div>
        );
      })}
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-slate-50 last:border-0">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-xs sm:text-sm font-semibold text-slate-800">{layanan.nama_layanan || "-"}</p>
        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
          {layanan.tipe_layanan && (
            <p className="text-[10px] sm:text-[11px] text-slate-400 capitalize">{layanan.tipe_layanan}</p>
          )}
          {durasi > 0 && (
            <p className="text-[10px] sm:text-[11px] text-slate-500 font-medium">{durasi} menit</p>
          )}
        </div>
      </div>
      <p className="text-xs sm:text-sm font-bold text-slate-700 flex-shrink-0">{formatCurrency(harga)}</p>
    </div>
  );
}
...
function ChatModal({ isOpen, onClose, bookingId, nakesName, nakesPhoto }) {
  ...
  const normalizeMessage = (msg, idx = 0) => {
    ...
    const isPatient = senderRaw === "pasien" || senderRaw === "patient" || senderRaw === "user" || senderRaw === "client";
  ...
  useEffect(() => {
    ...
    const initWS = async () => {
      try {
              ...
              if (incoming) {
                setMessages((prev) => {
                  const isDuplicate = prev.some(
                    (p) => p.id === incoming.id || (p.text === incoming.text && p.sender === incoming.sender)
                  );
        ...
        socket.onerror = (e) => console.warn("WebSocket chat error:", e);
      } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-xs p-0 sm:p-4">
      <div className="w-full max-w-md bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col h-[85vh] sm:h-[600px] overflow-hidden">
        <div className="bg-emerald-600 px-4 py-3.5 flex items-center justify-between text-white shadow-sm">
          ...
          <button onClick={onClose} className="p-1.5 rounded-full hover:bg-white/20 text-white transition active:scale-95 cursor-pointer" aria-label="Tutup Chat">
          ...
          {messages.map((m) => {
            if (m.sender === "system") {
              return (
                <div key={m.id} className="text-center my-2">
                  <span className="inline-block px-3 py-1 bg-slate-200/70 text-slate-600 rounded-full text-[11px]">{m.text}</span>
                </div>
              );
            }
            ...
            return (
              <div key={m.id} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                ...
                <div className={`max-w-[78%] px-3.5 py-2 rounded-2xl text-xs sm:text-sm leading-relaxed shadow-xs ${isMe ? "bg-emerald-600 text-white rounded-br-xs" : "bg-white text-slate-800 border border-slate-200/80 rounded-bl-xs"}`}>
                  <p className="break-words">{m.text}</p>
                  <p className={`text-[9px] mt-1 text-right ${isMe ? "text-emerald-200" : "text-slate-400"}`}>{m.time}</p>
                </div>
              </div>
            );
          })}
        ...
        <div className="px-3 py-2 bg-white border-t border-slate-100 flex gap-1.5 overflow-x-auto no-scrollbar">
          {["Halo, apakah sudah dekat?", "Saya menunggu di depan rumah ya", "Terima kasih!"].map((text) => (
            <button key={text} type="button" onClick={() => setInputMessage(text)} className="text-[11px] px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-full shrink-0 transition active:scale-95 cursor-pointer">
              {text}
            </button>
          ))}
        </div>
        ...
        <form onSubmit={handleSend} className="p-3 bg-white border-t border-slate-100 flex items-center gap-2">
          ...
          />
          <button type="submit" disabled={!inputMessage.trim() || isSending} className="w-10 h-10 rounded-full bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 text-white flex items-center justify-center transition active:scale-95 shrink-0 cursor-pointer" aria-label="Kirim Pesan">
...
function BookingAktifContent() {
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-6 text-center gap-5">
        ...
        <button onClick={() => router.push("/")} className="px-6 py-2.5 rounded-full bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700 active:scale-95 transition-all">
          Ke Beranda
        </button>
      </div>
    );
  ...
  const { tenaga_medis_tracking: nakes, tracking_info: info } = data;
  ...
  const totalLayanan = layananList.reduce(
    (sum, l) => sum + Number(l.sl ?? l.s1 ?? l.harga ?? 0),
    0
  );

  const bookingBhp = Array.isArray(booking.booking_bhp) ? booking.booking_bhp : [];
  const bhpTambahanItems = bookingBhp.filter((item) => Number(item?.qty_tambahan ?? 0) > 0);

  const biayaTambahanNominalFinal = Number(
    biayaTambahan?.nominal ??
      bhpTambahanItems.reduce(
        (sum, item) =>
          sum +
          Number(
            item?.total_sb_tambahan ??
              Number(item?.qty_tambahan ?? 0) * Number(item?.harga_jual ?? 0)
          ),
        0
      )
  );

  const biayaTambahanStatus =
  ...
  const handleBayarBhp = () => {
    ...
    router.push(
      `/pembayaran/pilih-metode?booking_id=${encodeURIComponent(
        booking.id_booking
      )}&booking_code=${encodeURIComponent(
        bookingCode
      )}&total=${encodeURIComponent(
        biayaTambahanNominalFinal
      )}&type=bhp`
    );
  };
    ...
    <div className="min-h-screen bg-slate-50" style={{ fontFamily: '"Poppins","Inter","Segoe UI",sans-serif' }}>
      ...
      <div className={`bg-gradient-to-r ${cfg.accent} px-4 sm:px-6 lg:px-8 xl:px-12 pt-5 pb-5 relative rounded-b-2xl sm:rounded-b-3xl shadow-sm text-white`}>
        <div className="max-w-lg lg:max-w-6xl xl:max-w-7xl mx-auto flex items-center justify-between gap-3">
          <button type="button" onClick={() => router.back()} className="w-9 h-9 rounded-full bg-white/20 hover:bg-white/30 backdrop-blur-sm flex items-center justify-center shrink-0 transition active:scale-90 cursor-pointer" aria-label="Kembali">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        <span className={`font-bold font-mono tracking-tight text-xs ${isSelected ? "text-white" : "text-emerald-800"}`}>{bCode}</span>
                        {sName ? (
                          <p className={`text-[11px] truncate mt-0.5 ${isSelected ? "text-emerald-100" : "text-slate-500"}`}>{sName}</p>
                        ) : null}
                      </div>
                    </div>
              ...
              <Section>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button type="button" onClick={() => setIsChatOpen(true)} className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-emerald-50 hover:bg-emerald-100 border border-emerald-300 text-emerald-700 flex items-center justify-center shadow-xs active:scale-90 transition-all cursor-pointer" title="Chat Tenaga Medis" aria-label="Chat Tenaga Medis">
                    ...
                    {nakesData.no_telp && (
                      <a href={`tel:${nakesData.no_telp}`} className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-emerald-600 hover:bg-emerald-700 text-white flex items-center justify-center shadow-md active:scale-90 transition-all cursor-pointer" title="Telepon Tenaga Medis" aria-label="Telepon Tenaga Medis">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-4 h-4 sm:w-5 sm:h-5">
              ...
              {layananList.length > 0 ? (
                <>
                  ...
                  <div className="flex justify-between items-center pt-3 mt-1 border-t border-slate-100">
                    <span className="text-xs sm:text-sm font-semibold text-slate-500">Total Layanan</span>
            ...
            <Section title="Biaya Tambahan BHP">
              {biayaTambahanNominalFinal > 0 ? (
                <>
                  {bhpTambahanItems.length > 0 && (
                    <div className="space-y-1.5">
                      {bhpTambahanItems.map((item, idx) => {
                        const jumlah = Number(item.qty_tambahan ?? 0);
                        const harga = Number(item.harga_jual ?? 0);
                        const subtotal = Number(item.total_sb_tambahan ?? jumlah * harga);
                        return (
                          <div key={item.id_booking_bhp ?? `${item.id_bhp}-${idx}`} className="flex justify-between items-start gap-3 py-2 border-b border-slate-50 last:border-0">
                            <div className="min-w-0">
                              <p className="text-xs sm:text-sm font-semibold text-slate-800">{item.nama_bhp}</p>
                              <p className="text-[10px] sm:text-[11px] text-slate-400">Tambahan {jumlah} × {formatCurrency(harga)}</p>
                            </div>
                            <p className="text-xs sm:text-sm font-bold text-slate-700 shrink-0">{formatCurrency(subtotal)}</p>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <div className="flex items-center justify-between gap-3 pt-3 mt-1 border-t border-slate-100">
                    <span className="text-xs sm:text-sm font-semibold text-slate-500">Total Tambahan</span>
                    <span className="text-sm font-bold text-amber-600">{formatCurrency(biayaTambahanNominalFinal)}</span>
                  </div>

                  <div className="flex items-center justify-between gap-3 mt-2">
                    <span className="text-xs sm:text-sm text-slate-500">Status Pembayaran</span>
                    <span className={`text-[11px] sm:text-xs font-semibold px-2 py-1 rounded-full ${!needsPayment ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                      {!needsPayment ? "Lunas" : "Belum Bayar"}
                    </span>
                  </div>
                ...
                <p className="text-xs sm:text-sm text-slate-400">
                  Belum ada biaya tambahan dari tenaga medis.
                </p>
              )}
            </Section>
            ...
            <Section title="Detail Pesanan">
              ...
              {biayaTambahanNominalFinal > 0 && (
                <InfoRow
                  ...
                  value={`${formatCurrency(biayaTambahanNominalFinal)}${!needsPayment ? " (Lunas)" : " (Belum Bayar)"}`}
                />
              )}
