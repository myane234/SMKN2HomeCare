...
export default function BookingDetailSheet({
  ...
  const checkingBiayaTambahanRef = useRef(false);

  const [bhpItems, setBhpItems] = useState([]);
  const [savedBhpSnapshot, setSavedBhpSnapshot] = useState([]);
  const [hasBhpChanges, setHasBhpChanges] = useState(false);
  ...
  const sheetHeight = isFullscreen
    ...
    : Math.max(
        380,
        (viewport?.height ?? 800) - MOBILE_NAV_HEIGHT - 12
      );

  const peekHeight = isDesktop
    ? DESKTOP_PEEK_HEIGHT
    : MOBILE_PEEK_HEIGHT;
  ...
  const peekOffset = isFullscreen
    ...
    : Math.max(
        0,
        (viewport?.height ?? 800) * 0.9 - peekHeight
      );

  const makeKey = (idLayanan, idBhp) =>
    `${idLayanan ?? "x"}_${idBhp ?? "x"}`;
  ...
  const createBhpSnapshot = (items) =>
    (Array.isArray(items) ? items : [])
      .map((item) => ({
        id_booking_bhp: item?.id_booking_bhp ?? null,
        id_layanan: item?.id_layanan ?? null,
        id_bhp: item?.id_bhp ?? null,
        qty_real: Number(item?.qty_real) || 0,
      }))
      .sort((a, b) =>
        `${a.id_layanan}_${a.id_bhp}_${a.id_booking_bhp}`.localeCompare(
          `${b.id_layanan}_${b.id_bhp}_${b.id_booking_bhp}`
        )
      );

  const isSameBhpSnapshot = (items, snapshot) =>
    JSON.stringify(createBhpSnapshot(items)) ===
    JSON.stringify(snapshot || []);

  const isBhpLocked =
    normalizeStatus(
      biayaTambahanStatus ||
        bhpMeta.status_pembayaran_bhp
    ) === "Lunas";
  ...
  const buildSkeletonFromBooking = useCallback(() => {
    ...
    if (Array.isArray(booking?.layanan_items)) {
      booking.layanan_items.forEach((layanan) => {
        const idLayanan =
          layanan?.id_layanan ?? layanan?.id ?? null;
        ...
        if (!Array.isArray(bhpList)) return;
  ...
  const parseBhpResponse = useCallback(
    (response) => {
      ...
      const { list: skeletonList, map: skeletonMap } =
        buildSkeletonFromBooking();

      let itemsSource = [];

      if (Array.isArray(root?.items)) {
        itemsSource = root.items;
      } else if (Array.isArray(root?.bhp_items)) {
        itemsSource = root.bhp_items;
      ...
      itemsSource.forEach((entry) => {
        ...
        const idBhp =
          entry?.id_bhp ??
          entry?.bhp_item?.id_bhp ??
          entry?.id ??
          null;

        if (idBhp == null) return;

        const qtyTambahanRaw =
          entry?.qty_tambahan ??
          entry?.jumlah_tambahan ??
          null;
        ...
        if (!targetKey) {
          for (const [key, value] of skeletonMap) {
            if (
              value.id_bhp === idBhp &&
              !usedKeys.has(key)
            ) {
        ...
        if (targetKey) {
          ...
          const item = skeletonMap.get(targetKey);

          if (qtyRealRaw != null) {
            item.qty_real = Math.max(
              item.qty_default,
              Number(qtyRealRaw) || 0
            );
          } else {
            item.qty_real =
              ...
              Math.max(
                0,
                Number(qtyTambahanRaw) || 0
              );
          }

          item.qty_tambahan = Math.max(
            0,
            item.qty_real - item.qty_default
          );

          if (entry?.id_booking_bhp) {
            item.id_booking_bhp =
              entry.id_booking_bhp;
          }
          ...
          const newItem = {
            id_booking_bhp:
              entry?.id_booking_bhp ?? null,
            id_layanan: idLayanan,
            nama_layanan:
              entry?.nama_layanan ?? null,
            id_bhp: idBhp,
            nama_bhp:
              entry?.nama_bhp ??
              entry?.bhp_item?.nama_bhp ??
              entry?.nama_item ??
              "-",
            harga_satuan:
              Number(
      ...
      const calculatedTotal =
        skeletonList.reduce(
          (acc, item) =>
            acc +
            (Number(item.qty_tambahan) || 0) *
              (Number(item.harga_satuan) || 0),
          0
        );

      const rawTotalFromMeta =
        root?.total_tambahan ??
        root?.sb_tambahan ??
        response?.meta?.total_tambahan;

      let totalTambahan = calculatedTotal;

      if (
        rawTotalFromMeta != null &&
        Number(rawTotalFromMeta) > 0
      ) {
        totalTambahan = Math.max(
          calculatedTotal,
          Number(rawTotalFromMeta)
        );
      }
  ...
  /* =========================================================
   * EXTRACT STATUS BHP DARI DETAIL ORDER
   * =======================================================*/
  const extractBiayaTambahanFromResponse =
    useCallback((response) => {
      ...
      return {
        ...
        order_id:
          tambahan?.order_id ?? null,
        kode_booking_tambahan:
          tambahan?.kode_booking ?? null,
        waktu_bayar:
          tambahan?.waktu_bayar ?? null,
      };
    }, []);

  /* =========================================================
   * LOAD STATUS PEMBAYARAN
   * =======================================================*/
  const loadBiayaTambahanStatus =
    useCallback(async () => {
      if (
        !bookingId ||
        checkingBiayaTambahanRef.current
      ) {
        return;
      }

      checkingBiayaTambahanRef.current = true;
      setCheckingBiayaTambahan(true);

      try {
        const response =
          await getNakesOrderDetail(
            bookingId
          );

        const parsed =
          extractBiayaTambahanFromResponse(
            response
          );
        ...
        if (parsed.status) {
          setBiayaTambahanStatus(
            parsed.status
          );
        }
      } catch (error) {
        console.warn(
          "Gagal mengecek status biaya tambahan:",
          error
        );
      } finally {
        checkingBiayaTambahanRef.current =
          false;
        setCheckingBiayaTambahan(
          false
        );
      }
    }, [
   ...
   * LOAD BHP
   * =======================================================*/
  const [loadedTargetCode, setLoadedTargetCode] =
    useState(null);

  useEffect(() => {
    if (!isOpen) {
      setLoadedTargetCode(null);
      setBiayaTambahanStatus(null);
      setBhpMeta({
        total_tambahan: 0,
        status_pembayaran_bhp: null,
      });
      setSavedBhpSnapshot([]);
      setHasBhpChanges(false);
    }
  }, [isOpen]);
  ...
  const {
    ...
    isOpen && targetCode
      ? `/api/nakes/booking/${encodeURIComponent(
          targetCode
        )}/bhp`
      : null,
  ...
  useEffect(() => {
    ...
    if (loadedTargetCode !== targetCode) {
      if (!rawBhpData) {
        const skeleton =
          buildSkeletonFromBooking().list;

        setBhpItems(skeleton);
        setSavedBhpSnapshot(
          createBhpSnapshot(skeleton)
        );
        setHasBhpChanges(false);
      ...
      setBhpMeta((prev) => ({
        ...prev,
        total_tambahan:
          result.total_tambahan,
        ...(result.status_pembayaran_bhp
          ? {
              status_pembayaran_bhp:
                result.status_pembayaran_bhp,
            }
  ...
  /* =========================================================
   * SYNC SAAT TAB KEMBALI TERLIHAT
   * TANPA POLLING
   * =======================================================*/
  useEffect(() => {
    if (!bookingId || !isOpen) return;
  ...
  /* =========================================================
   * SAVE BHP
   * TIDAK REFRESH PARENT / SWR SETELAH SAVE
   * =======================================================*/
  ...
  const hasAdditionalCost =
    effectiveTotalTambahan >
    0;

  const isBiayaTambahanLunas =
    !hasAdditionalCost ||
    paymentStatus === "Lunas";
                ...
                <div className="p-3.5 rounded-2xl bg-white border border-slate-100 shadow-sm space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isBhpLocked && (
                        <span className="inline-flex items-center gap-1.5 text-[10px] font-bold px-2 py-1 rounded-full bg-slate-100 text-slate-500 border border-slate-200">
